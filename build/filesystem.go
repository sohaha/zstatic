...
func FindGoFiles(directory string, skipDirs []string) ([]string, error) {
	...
	err := filepath.Walk(directory,
		func(path string, info os.FileInfo, err error) error {
			...
			for _, s := range skipDirs {
				if zfile.RealPath(s) == path {
					return filepath.SkipDir
				}
			}
			if basePath == "node_modules" || path == zfile.RealPath("./vendor") {
				return filepath.SkipDir
			}
...
func GetBinFiles(args []string, ignoreErrors bool, skipDirs []string) (mewnFiles []string, err error) {
	var goFiles []string
	if len(args) > 0 {
		...
		goFiles, err = FindGoFiles(cwd, skipDirs)
		if err != nil && !ignoreErrors {
			return
		}
	}
